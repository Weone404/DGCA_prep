...
function collectChapterIds(subject) {
  if (!subject) return []
  if (subject.chapterIds === null) return Object.keys(ALL_QUESTION_BANKS)

  const collected = []
  if (Array.isArray(subject.chapterIds)) {
    collected.push(...subject.chapterIds)
  }
  const nestedGroups = [subject.children, subject.subcategories]
...
export function buildQuestionPool(subjectId, targetCount = DEFAULT_MOCK_TEST_SETTINGS.count) {
  ...
  const warning = allQuestions.length < targetCount
    ? `Only ${allQuestions.length} questions are available for this subject. The test will use all of them.`
    : ''
