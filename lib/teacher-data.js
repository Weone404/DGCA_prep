...
/**
 * Attendance is two states: present and absent. Historic rows may still carry
 * 'late', so fold those into present at read time — no schema change, no writes.
 */
function foldAttendanceStatus(status) {
...
export async function getStudentAttendance(email) {
  await ensureTeacherDataTables()
  const normalizedEmail = getLowerEmail(email)
  if (!normalizedEmail) return { present: 0, absent: 0, monthPct: 0, days: {}, recent: [] }
  ...
  const summary = summaryResult.rows.reduce((acc, row) => {
    const status = foldAttendanceStatus(row.status)
    acc[status] = (acc[status] || 0) + 1
    return acc
  }, { present: 0, absent: 0 })

  // Leave sits out of both sides: it must not reduce the percentage.
  const rated = summary.present + summary.absent
  const monthPct = rated ? Math.round((summary.present / rated) * 100) : 0
  const days = summaryResult.rows.reduce((acc, row) => {
    acc[row.date ? String(row.date) : ''] = foldAttendanceStatus(row.status)
    return acc
  }, {})
  ...
  return {
    ...
    recent: recentResult.rows.map((row) => ({
      date: row.date ? String(row.date) : '',
      batch: row.batch || '',
      status: foldAttendanceStatus(row.status),
      note: row.note || '',
    })),
  }
}
