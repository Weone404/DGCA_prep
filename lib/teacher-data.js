...
export async function getStudentsWithResults() {
  ...
  const [usersResult, normalResultsResult, assignedResultsResult] = await Promise.all([
    pool.query(`SELECT id, name, email, phone, batch, COALESCE(whatsapp_opt_in, false) AS whatsapp_opt_in, whatsapp_opt_in_at, created_at FROM users ORDER BY created_at ASC, name ASC`),
  ...
  let students = usersResult.rows.map((user) => {
