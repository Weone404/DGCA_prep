...
export async function getStudentsWithResults() {
  ...
  const [usersResult, normalResultsResult, assignedResultsResult] = await Promise.all([
    pool.query(`SELECT id, name, email, phone, batch, created_at FROM users ORDER BY created_at ASC, name ASC`),
    pool.query(`SELECT id, student_email, chapter_id, score, total, answers, date FROM test_results ORDER BY date DESC, id DESC`),
    pool.query(`SELECT atr.id, atr.test_id, atr.student_email, atr.student_name, atr.score, atr.total, atr.answers, atr.submitted_at, at.title AS test_title, at.subject_label FROM assigned_test_results atr LEFT JOIN assigned_tests at ON at.id = atr.test_id ORDER BY atr.submitted_at DESC, atr.id DESC`),
  ])

  const normalByEmail = new Map()
  for (const row of normalResultsResult.rows) {
  ...
  for (const row of assignedResultsResult.rows) {
    const key = getLowerEmail(row.student_email)
...
export async function addStudent(payload) {
  ...
  await pool.query(
    `INSERT INTO users (name, email, phone, batch, created_at) VALUES ($1, $2, $3, $4, NOW()) ON CONFLICT (email) DO UPDATE SET name = EXCLUDED.name, phone = EXCLUDED.phone, batch = EXCLUDED.batch`,
...
export async function listAssignedTests({ onlyActive = false } = {}) {
  ...
  const result = await pool.query(`SELECT id, title, subject_id, subject_label, chapter_id, chapter_label, chapter_ids, num_questions, duration_mins, instructions, is_active, created_at FROM assigned_tests ${whereClause} ORDER BY created_at DESC`)
...
export async function createAssignedTest(payload) {
  await ensureTeacherDataTables()
  const result = await pool.query(`
    INSERT INTO assigned_tests (title, subject_id, subject_label, chapter_id, chapter_label, chapter_ids, num_questions, duration_mins, instructions, is_active, created_at)
